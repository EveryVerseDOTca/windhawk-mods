...
static void RefreshProgramsList() {
    ...
    if (g_listHwnd) {
        ...
        for (const auto& exeLower : g_programs) {
            SendMessageW(g_listHwnd, LB_ADDSTRING, 0, (LPARAM)exeLower.c_str());
        }
        if (!g_programs.empty())
            SendMessageW(g_listHwnd, LB_SETCURSEL, 0, 0);
    }
}
...
static void ApplySelectedRuleFromUi() {
    ...
    const int progSel = (int)SendMessageW(g_listHwnd, LB_GETCURSEL, 0, 0);
    const int monSel = (int)SendMessageW(g_comboHwnd, CB_GETCURSEL, 0, 0);
    if (progSel == LB_ERR || monSel == CB_ERR)
        return;
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            CreateWindowExW(
                ...
                WS_CHILD | WS_VISIBLE, margin, margin + 270, 490, 18, hwnd,
                nullptr, nullptr, nullptr);
